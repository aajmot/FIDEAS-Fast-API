...
class PurchaseInvoice(Base):
    """Purchase Invoice - records supplier invoices with GST and currency support"""
    __tablename__ = 'purchase_invoices'
    __table_args__ = (
        UniqueConstraint('invoice_number', 'tenant_id', name='uq_invoice_number_tenant'),
        CheckConstraint(
        ...
        CheckConstraint(
            'tax_amount_base = cgst_amount_base + sgst_amount_base + igst_amount_base + ugst_amount_base + cess_amount_base',
            name='chk_gst_sum'
        ),
        CheckConstraint(
    ...
    reference_number = Column(String(50))  # Supplier invoice number
    invoice_date = Column(Date, nullable=False)
    due_date = Column(Date)
    
    # References
    supplier_id = Column(Integer, ForeignKey('suppliers.id', ondelete='RESTRICT'), nullable=False)
    purchase_order_id = Column(Integer, ForeignKey('purchase_orders.id', ondelete='SET NULL'))
...
class PurchaseInvoiceItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            'tax_amount_base = cgst_amount_base + sgst_amount_base + igst_amount_base + ugst_amount_base + cess_amount_base',
            name='chk_gst_components'
        ),
        CheckConstraint(
            "(cgst_amount_base > 0 AND sgst_amount_base > 0 AND igst_amount_base = 0 AND ugst_amount_base = 0) "
            "OR (igst_amount_base > 0 AND cgst_amount_base = 0 AND sgst_amount_base = 0 AND ugst_amount_base = 0) "
            "OR (ugst_amount_base > 0 AND cgst_amount_base = 0 AND sgst_amount_base = 0 AND igst_amount_base = 0) "
            "OR (cgst_amount_base = 0 AND sgst_amount_base = 0 AND igst_amount_base = 0 AND ugst_amount_base = 0)",
            name='chk_gst_exclusivity'
        ),
        {'extend_existing': True}
    )
    ...
    # Costing
    landed_cost_per_unit = Column(Numeric(15, 4))

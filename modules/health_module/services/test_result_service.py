...
class TestResultService:
    ...
    def get_paginated(self, tenant_id, page, per_page, search=None):
        try:
            with db_manager.get_session() as session:
                ...
                if search:
                    query = query.filter(or_(
                        TestResult.result_number.ilike(f"%{search}%"),
                        TestResult.performed_by.ilike(f"%{search}%"),
            ...
            logger.error(f"Error fetching paginated test results: {str(e)}", self.logger_name)
            raise
